...
export async function getNotificationsByOfficer(officerId: string): Promise<Notification[]> {
  const { data } = await http.get<Notification[]>(`/Notification/officer/${officerId}`);
  return data;
}
...
export async function markNotificationAsRead(notificationId: number): Promise<void> {
  await http.put(`/Notification/${notificationId}/read`);
}
...
export async function getNotificationsByCitizen(citizenId: string): Promise<Notification[]> {
  const { data } = await http.get<Notification[]>(`/Notification/citizen/${citizenId}`);
  return data;
}
